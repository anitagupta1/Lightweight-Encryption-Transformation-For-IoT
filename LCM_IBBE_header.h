...
SETUPVALS lcm_ibbe_setup(pairing_t pairing, int m) {
    ...
    // element_set(setup_vals.alpha_powers[0], setup_vals.alpha);  // α^2 = α * α
    ...
    // Calculate h^α, h^(α^2), ..., h^(α^m) and store in h_powers
    element_pow_zn(params->h_powers[0], params->h, setup_vals.alpha);  // h^α
